...
async function loadRealDepths() {
    try {
        const res = await fetch('http://127.0.0.1:8000/api/depths');
...
async function renderActiveLayer(dayIndex) {
    ...
    try {
        ...
        let url = `http://127.0.0.1:8000/api/${activeDataType}`;
...
window.buildUnifiedSidebar = async function(lat, lon, zonasiProps = null) {
            ...
            html += `
                <div class="mt-3">
                    <div class="flex justify-between items-center">
                        ...
                        <a href="http://127.0.0.1:8000/api/export-csv?lat=${lat}&lon=${lon}&param=${moData.type}&mode=timeseries" target="_blank" class="text-[9px] bg-green-600 text-white px-2 py-0.5 rounded hover:bg-green-700 transition cursor-pointer flex items-center gap-1 shadow-sm">📥 CSV</a>
                    </div>
                ...
                html += `
                    <div class="mt-3 border-t border-gray-100 pt-3">
                        <div class="flex justify-between items-center">
                            ...
                            <a href="http://127.0.0.1:8000/api/export-csv?lat=${lat}&lon=${lon}&param=${moData.type}&mode=depth" target="_blank" class="text-[9px] bg-green-600 text-white px-2 py-0.5 rounded hover:bg-green-700 transition cursor-pointer flex items-center gap-1 shadow-sm">📥 CSV</a>
                        </div>
    ...
    if (moData && moData.type !== 'batimetri') {
        ...
        if (['suhu', 'salinitas', 'arus'].includes(moData.type)) {
            try {
                const response = await fetch(`http://127.0.0.1:8000/api/profile?lat=${lat}&lon=${lon}&param=${moData.type}`);
...
async function renderThermalFront(dayIndex) {
    ...
    try {
        const res = await fetch(`http://127.0.0.1:8000/api/thermal-front/${dayIndex}`);
        const data = await res.json();
...
async function buildTideSidebar(stationName, lat, lon, stationCode) {
    ...
    try {
        ...
        const res = await fetch(`http://127.0.0.1:8000/api/tide?station_code=${stationCode}`);
        const data = await res.json();
        ...
        let html = `
            <div class="border-b pb-2 mb-3 border-blue-100 flex justify-between items-start">
                ...
                <a href="http://127.0.0.1:8000/api/tide?lat=${lat}&lon=${lon}" target="_blank" download="Tide_${stationName.replace(/\s+/g, '_')}.json" class="text-[9px] bg-green-600 text-white px-2 py-1 rounded hover:bg-green-700 transition cursor-pointer shadow-sm">📥 JSON</a>
            </div>
